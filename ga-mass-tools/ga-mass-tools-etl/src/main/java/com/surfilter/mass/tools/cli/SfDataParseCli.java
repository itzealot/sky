...
import com.surfilter.mass.tools.thread.SfDataTransferThread;
import com.surfilter.mass.tools.util.FileUtil;
import com.surfilter.mass.tools.util.Threads;

/**
 * 对身份关系数据进行解析并生成 .log 和 .log.ok 文件
 * 
 * @author zealot
 *
 */
public class SfDataParseCli {
	private static final Logger LOG = LoggerFactory.getLogger(SfDataParseCli.class);

	public static void main(String[] args) {
		LOG.info("身份关系数据处理手动任务开始,在配置文件conf/conf.properties中修改配置参数...");
		...
		SfDataTransferThread[] threads = new SfDataTransferThread[poolSize];
		...
		try {
			...
			for (int i = 0; i < poolSize; i++) {
				threads[i] = new SfDataTransferThread(queue, conf);
			...
			LOG.error("身份关系数据处理手动任务手动任务失败!", e);
		}
