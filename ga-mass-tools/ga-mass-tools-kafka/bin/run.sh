...
print_usage ()
{
  ...
  echo "  -> RelationParseApp"
  echo "  -> KafkaConsumerApp"
  exit 1
}
...
if  [ "$COMMAND" = "RelationParseApp" ];then
  CLASS=com.surfilter.gamass.RelationParseApp
  nohup "$JAVA" -Djava.awt.headless=true $HEAP_OPTS -classpath "$CLASSPATH" $CLASS > $STDOUT_FILE 2>&1 &
  
elif  [ "$COMMAND" = "KafkaConsumerApp" ];then
  CLASS=com.surfilter.gamass.KafkaConsumerApp
  nohup "$JAVA" -Djava.awt.headless=true $HEAP_OPTS -classpath "$CLASSPATH" $CLASS > $STDOUT_FILE 2>&1 &
