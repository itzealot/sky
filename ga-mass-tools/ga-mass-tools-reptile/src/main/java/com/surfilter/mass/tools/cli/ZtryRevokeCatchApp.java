...
/**
 * Ztry 撤销查询的详情 Url及详细信息爬虫程序
 * 
 * @author zealot
 *
 */
public class ZtryRevokeCatchApp {

	static final Logger LOG = LoggerFactory.getLogger(ZtryRevokeCatchApp.class);
	...
	public static void main(String[] args) {
		...
		for (int year = startYear; year <= endYear; year++) {
			for (int month = 1; month <= 12; month++) { // month
				fetchUrlAndDetail(config, urlsDst, detailDst, year, month, true);
				Threads.sleep(1000);
			}
		}
	}
	 ...
	 * @param month
	 */
	private static void fetchUrlAndDetail(ZtryRevokeConfig config, String urlsDst, String detailDst, int year,
			int month, boolean isRevoke) {
		...
		fetchUrlAndDetail(config.getZtryCookies(), url, FileUtil.date2Path(urlsDst, start1, end1),
				FileUtil.date2Path(detailDst, start1, end1), true);
	...
	public static void fetchUrlAndDetail(Map<String, String> cookie, String url, String urlsFile, String detailsFile,
		...
		ZtryCatchUtil.fetchUrlAndDetail(url, new File(urlsFile), new File(detailsFile), 0, info, ieRevoke);
	}
}
