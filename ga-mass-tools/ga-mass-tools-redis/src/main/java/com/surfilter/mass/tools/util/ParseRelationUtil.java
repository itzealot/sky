...
import com.surfilter.mass.tools.entity.CertificationTrack;
...
public class ParseRelationUtil {
	...
	public static void readCertification2RowKey(BlockingQueue<String> queue, File file, long sleep, int counts)
			throws Exception {
		BufferedReader reader = null;
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				String res = parseCertification(line);
				if (res != null)
					queue.put(res);
				index++;
	...
	public static String parseCertification(String line) {
		String[] arrays = line.split("\t");
		if (arrays.length != 2) {
			LOG.error("error length line:" + line);
			return null;
	 ...
	 * @return
	 */
	public static String joinRelationRowKey(Relation r) {
		return new StringBuffer(128).append(r.getIdFrom()).append(CP).append(r.getFromType()).append(CP)
	 ...
	 * @return
	 */
	public static String joinRelationReverseRowKey(Relation r) {
		return new StringBuffer(128).append(r.getIdTo()).append(CP).append(r.getToType()).append(CP)
	 ...
	 * @return
	 */
	public static String joinCertificationRowKey(Certification c) {
		return new StringBuffer(64).append(c.getId()).append(CP).append(c.getIdType()).toString();
	}
	...
	public static void parseAndWriteRelation(List<Relation> msgs, String relationDir, String hbaseParams,
		...
		insertRelation2Hbase(msgs, hbaseParams, relationTableName);
	}
	...
	public static void insertRelation2Hbase(List<Relation> msgs, String hbaseParams, String relationTableName) {
		...
		for (Relation r : msgs) {
			rowKeys.add(ParseRelationUtil.joinRelationRowKey(r));
			values.add(ParseRelationUtil.joinRelationValues(r));
		}

		HbaseUtil.migrateRelation2Hbase(hbaseParams, rowKeys, values, relationTableName);
	...
	public static void parseAndWriteCertification(List<Relation> msgs, String certDir, String hbaseParams,
			String cetificationTableName, String serversInfo) {
		...
		insertCertification2HbaseAndRedis(certs, hbaseParams, cetificationTableName, serversInfo);
	...
	public static void insertCertification2HbaseAndRedis(List<Certification> certs, String hbaseParams,
		...
		for (Certification c : certs) {
			rowKeys.add(ParseRelationUtil.joinCertificationRowKey(c));
		...
		RedisUtil.updateCertificationInRedis(rowKeys, values, serversInfo);
