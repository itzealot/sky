...
public final class HbaseUtil {
	...
	public static void migrateRelation2Hbase(String hbaseParams, List<String> rowKeys, List<String> values,
		...
		List<Put> puts = getsRelationFromHbase(dao, tableName, rowKeys, values);
		...
		try {
			...
			if (conn != null) {
				...
				for (int i = 0; i < size; i++) {
					...
					put.addColumn(CF_NAME_BYTES, col, INT_DEFALT_VALUE_BYTES);
					...
					if (results[i] != null && !results[i].isEmpty()) { // 行健存在
						...
						if (val == null || Bytes.toLong(val) > lastStartTime) { // 比较首次发现时间
							put.addColumn(CF_NAME_BYTES, FIRST_START_TIME_BYTES,
									Bytes.toBytes(Long.valueOf(lastStartTime)));
							put.addColumn(CF_NAME_BYTES, FIRST_TERMINAL_NUM_BYTES, lastTerminalNumBytes);
						}
					} else { // 行健不存在，代表是第一次插入，插入次数
						put.addColumn(CF_NAME_BYTES, DISCOVER_TIMES_BYTES, LONG_DEFALT_VALUE_BYTES);
					...
					if (flag) { // 是否更新最新发现时间以及场所
						put.addColumn(CF_NAME_BYTES, ltnCol, lastTerminalNumBytes);
						put.addColumn(CF_NAME_BYTES, lstCol, lastStartTimeBytes);
					}
		...
		try {
			...
			if (conn != null) {
				...
				for (int i = 0; i < size; i++) {
					...
					byte[] rowKeyBytes = Bytes.toBytes(rowKey);
					byte[] reverseRowKeyBytes = Bytes.toBytes(reverseRelationKey(rowKey));
					...
					reversePut.addColumn(CF_NAME_BYTES, col, INT_DEFALT_VALUE_BYTES);
					...
					if (results[i] != null && !results[i].isEmpty()) { // 行健存在
						...
						put.addColumn(CF_NAME_BYTES, DISCOVER_TIMES_BYTES, LONG_DEFALT_VALUE_BYTES);
						reversePut.addColumn(CF_NAME_BYTES, DISCOVER_TIMES_BYTES, LONG_DEFALT_VALUE_BYTES);
						flag = true;
					}
					...
					if (flag) { // 是否更新最新发现时间以及场所
						...
						reversePut.addColumn(CF_NAME_BYTES, ltnCol, lastTerminalNumBytes);
						reversePut.addColumn(CF_NAME_BYTES, lstCol, lastStartTimeBytes);
					}
	...
	private static String reverseRelationKey(String str) {
		String[] keyArr = str.split("\\|");
		return new StringBuffer(128).append(keyArr[2]).append(CP).append(keyArr[3]).append(CP).append(keyArr[0])
				.append(CP).append(keyArr[1]).toString();
	}
