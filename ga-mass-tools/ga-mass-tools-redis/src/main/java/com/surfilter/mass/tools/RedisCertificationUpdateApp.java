...
import com.surfilter.mass.tools.task.RedisCertificationUpdateTask;
import com.surfilter.mass.tools.util.ParseRelationUtil;
import com.surfilter.mass.tools.util.Threads;

/**
 * redis certification value update client
 * 
 * @author zealot
 *
 */
public class RedisCertificationUpdateApp {

	private static final Logger LOG = LoggerFactory.getLogger(RedisCertificationUpdateApp.class);
	...
	public static void main(String[] args) throws Exception {
		...
		RedisCertificationUpdateTask[] tasks = new RedisCertificationUpdateTask[poolSize];
		ExecutorService executor = Executors.newFixedThreadPool(poolSize);

		for (int i = 0; i < poolSize; i++) {
			tasks[i] = new RedisCertificationUpdateTask(queue, conf);
