...
import com.surfilter.mass.tools.task.RedisCertificationChangeTask;
import com.surfilter.mass.tools.util.ParseRelationUtil;
import com.surfilter.mass.tools.util.Threads;

/**
 * redis certification value modify client
 * 
 * @author zealot
 *
 */
public class RedisCertificationChangeApp {

	private static final Logger LOG = LoggerFactory.getLogger(RedisCertificationChangeApp.class);
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("poolSize:" + poolSize);
		System.out.println("prefixLen:" + conf.getInt(Constant.MAC_MODIFY_PREFIX_LENGTH, 6));

		RedisDaoAlias.getInstance(conf.get(Constant.WRITE_REDIS_IP) + "|" + conf.get(Constant.WRITE_REDIS_PORT));

		BlockingQueue<String> queue = new LinkedBlockingQueue<String>();
		File file = new File(src);
		RedisCertificationChangeTask[] tasks = new RedisCertificationChangeTask[poolSize];
		ExecutorService executor = Executors.newFixedThreadPool(poolSize);

		for (int i = 0; i < poolSize; i++) {
			tasks[i] = new RedisCertificationChangeTask(queue, conf);
