...
public class HBaseDao {
	...
	private HTableInterface getTable(HConnection conn, String tableName) throws IOException {
	...
	public void insertDatas(String tableName, List<Put> puts) throws IOException {
		...
		try {
			if (conn != null) {
				HTableInterface table = getTable(conn, tableName);
				if (table != null) {
					table.put(puts);
				} else {
	 ...
	 * conf.set("hbase.rpc.timeout", "60000");<br>
	 * conf.set("hbase.client.operation.timeout", "60000");// 单个操作执行时间总和<br>
	 * conf.set("hbase.client.scanner.caching", "1000");//
	 * scan单次rpc缓存的数据量，默认是100<br>
	 * conf.set("hbase.client.scanner.timeout.period", "60000");//
	 * scan单次rpc调用的过期时间<br>
	 * conf.set("hbase.client.retries.number", "2");// 失败重试次数 <br>
	 * 
	 * @param source_table
	 * @param dest_table
	 * @param column_family
	 ...
	 * @throws Exception
	 */
	public void tableTransport(HConnection conn, String source_table, String dest_table, String column_family,
			int batch, String rowStart) throws Exception {
		List<Put> puts = new ArrayList<Put>(batch);
		...
		try {
			if (conn != null) {
				HTableInterface table_src = getTable(conn, source_table);
				if (table_src != null) {
					// Iterator<Result> rst_it =
					// table_src.getScanner(cf).iterator(); // 根据行健获取结果
					Iterator<Result> iterator = table_src.getScanner(new Scan().setStartRow(rowStart.getBytes()))
							.iterator(); // 根据行健获取结果
					// Iterator<Result> rst_it =
					...
					if (!isCert && !isRel) {
						LOG.warn(
								"ATTENTION: SOURCE OR DEST TABLE IS NOT CERTIFICATION OR RELATION, IS GOING TO JUST COPY THE TABLE ! ! !");
					}
					byte[] rowKey = null;
					byte[] rowKeyNew = null;
					...
					while (iterator.hasNext()) {
						...
						try {// 读取数据
							...
							if (isCert) {
								rowKeyNew = row_key_split.length == 3 ? rowKey
										: MD5Hash.getMD5AsHex(rowKey).substring(0, 8).concat("|").concat(rowKeyStr)
												.getBytes();
							} else if (isRel) {
								rowKeyNew = row_key_split.length == 5 ? rowKey
										: MD5Hash
												.getMD5AsHex(row_key_split[0].concat("|").concat(row_key_split[1])
														.getBytes())
												.substring(0, 8).concat("|").concat(rowKeyStr).getBytes();
							} else {
							...
							for (byte[] col : col_val.keySet()) {
								put.add(cf, col, col_val.get(col));
							}
							puts.add(put);
							break_point++;
						...
						if (break_point % batch == 0) {
							...
							try {
								...
								LOG.error("insert hbase error,{}:{}-{} keys have inserted.exit...{}", source_table,
										break_point - batch, break_point, e);
								e.printStackTrace();
								throw e;
							}
	 ...
	 * @throws IOException
	 */
	public byte[] getByRowKey(String tableName, String columnFamily, String columnName, String rowKey)
			throws IOException {
		HConnection conn = getConn();
		byte[] family = Bytes.toBytes(columnFamily);
	 ...
	 * @throws IOException
	 */
	public List<byte[]> getByRowKeys(String tableName, String columnFamily, String columnName, List<String> rowKeys)
			throws IOException {
		HConnection conn = getConn();
		byte[] family = Bytes.toBytes(columnFamily);
		byte[] qualifier = Bytes.toBytes(columnName);
		...
		if (conn != null) {
			...
			if (table != null) {
				for (String rowkey : rowKeys) {
					table.incrementColumnValue(Bytes.toBytes(rowkey), Bytes.toBytes(cf), Bytes.toBytes(columnName),
	...
	public void modifyColumn(String tableName, List<String> rowkeys, byte[] family, String deleteColumn,
			String addColumn) throws IOException {
		HConnection conn = getConn();
		...
		try {
			if (conn != null) {
				...
				if (table != null) {
					...
					for (String s : rowkeys) {
						deletes.add(new Delete(Bytes.toBytes(s)).deleteColumns(family, delQualifier));
						puts.add(new Put(Bytes.toBytes(s)).add(family, addQualifier, Bytes.toBytes(1)));
					}
