 ...
 *******************************************************************************/
package com.sky.project.share.common.thread;
...
public final class Threads {
	...
	public static class WrapExceptionRunnable implements Runnable {
		...
		public WrapExceptionRunnable(Runnable runnable) {
			Assert.assertNotNull("runnable can't be null.", runnable);
	 ...
	 * 循环执行 Runnable 中的方法
	 */
	public static class WrapCircleRunnable implements Runnable {
		...
		/**
		 * WrapCircleRunnable, sleep [100, 200)ms
		 * 
		 * @param runnable
		 *            can't be null
		 */
		public WrapCircleRunnable(Runnable runnable) {
			this(runnable, DEFAULT_BOUND);
		}
		 ...
		 *            sleep [bound, 2 * bound)
		 */
		public WrapCircleRunnable(Runnable runnable, int bound) {
			Assert.assertNotNull("runnable can't be null.", runnable);
			this.runnable = runnable;
			this.bound = bound;
		}
		...
		public void run() {
			...
			while (true) {
				runnable.run();
				Threads.sleep(bound + random.nextInt(bound));
			}
		}
	}
