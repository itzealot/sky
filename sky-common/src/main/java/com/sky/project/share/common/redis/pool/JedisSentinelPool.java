 ...
 *******************************************************************************/
package com.sky.project.share.common.redis.pool;
...
import com.sky.project.share.common.redis.JedisTemplate;
import com.sky.project.share.common.redis.JedisUtils;
...
public final class JedisSentinelPool extends JedisPool {
	 ...
	 *            Array of HostAndPort to sentinel instances.
	 * @param masterName
	 *            One sentinel can monitor several redis master-slave pair, use
	 *            master name to identify them.
	 * @param masterConnectionInfo
	 *            The the other information like password,timeout.
	 * @param masterPoolConfig
	 *            Configuration of redis pool.
	 * 
	 */
	public JedisSentinelPool(HostAndPort[] sentinelAddresses, String masterName, ConnectionInfo masterConnectionInfo,
		...
		// Start MasterSwitchListener thread ,internal poll will be start in the
		// thread
	...
	/**
	 * Assert the argurment, throw IllegalArgumentException if the expression is
	 * true.
	 */
	...
	public class MasterSwitchListener extends Thread {
			...
			while (running.get()) {
				try {
					...
					if (sentinelPool != null) {
						...
						// When the system startup but the sentinels not yet,
						// init a urgly address to prevent null point
		...
		/**
		 * Pickup the first available sentinel, if all sentinel down, return
		 * null.
		 */
		...
		/**
		 * Subscriber for the master switch message from sentinel and init the
		 * new pool.
		 */
		private class MasterSwitchSubscriber extends JedisPubSub {
			@Override
			public void onMessage(String channel, String message) {
				// message example: +switch-master: mymaster 127.0.0.1 6379
				// 127.0.0.1 6380
				// +redirect-to-master default 127.0.0.1 6380 127.0.0.1 6381 (if
				// slave-master fail-over quick enough)
				logger.info("Sentinel " + sentinelPool.getAddress() + " published: " + message);
				String[] switchMasterMsg = message.split(" ");
				// if the master name equals my master name, destroy the old
				// pool and init a new pool
