...
import com.sky.project.share.common.redis.JedisTemplate.JedisActionNoResult;
import com.sky.project.share.common.redis.JedisTemplate.PipelineAction;
import com.sky.project.share.common.redis.JedisTemplate.PipelineActionNoResult;
 ...
 * 
 * Pass more than one JedisPool to the it, it will calculate which jedisPool
 * will handle the key. If only one jedisPool passed, it won't do the
 * calculation, so please use JedisShardedTemplate by default.
 * 
 * But it doesn't support multi-key actions by default, like: 1. Pipelined and
 * Transaction process multi-keys 2. Methods
 * in @redis.clients.jedis.MultiKeyCommands like mget, rpoplpush....
 * 
 * To support them, please use a common sharding key relate to these keys when
 * invoke the APIs.
 */
public class JedisShardedTemplate {
	...
	/**
	 * Hash the key and get the jedisTemplate from the hash ring. Get idea from
	 * Jedis's Sharded.java
	 */
	 ...
	 * Execute the action.
	 * 
	 * @key the action must process only this key, or this key is a sharding
	 * key.
	 */
	...
	public Set<String> zrevrangeByScore(final String shardingKey, final String key, final double max,
			final double min) {
		JedisTemplate jedisTemplate = getShard(shardingKey);
		return jedisTemplate.zrevrangeByScore(key, max, min);
	}
