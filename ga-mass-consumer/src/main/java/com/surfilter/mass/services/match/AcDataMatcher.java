...
import com.surfilter.mass.utils.NumberUtil;
...
public class AcDataMatcher extends AbstractDataMatcher {
    ...
    /**
     * 如果是ZDR，不做serviceRang以及areaRang过滤
     *
     * @param row
     * @return
     */
    public List<AlarmInfo> doMatch(String row) {
        ...
        if (arrays != null && arrays.length > 0) {
            // wl数据校验，第一个字段为mac，在ac之前匹配是为了把整条数据过滤，防止除了mac之外的身份继续报警
            if (arrays.length == context.getConf().getInt(ImcaptureConsts.WL_PARSER_KEY)
                    && (ImcaptureUtil.filter(this.context.getMacCompanyKeys(), helper.getMacFilterMap(), arrays) 
            		|| NumberUtil.parseLong(arrays[3]) > 1L)) {
                LOG.debug("wl mac: {} had been filtered", arrays[0]);
                return null;
            }
     ...
     * @return 如果是ZDR，直接返回true，不做区域和场所过滤
     */
    private boolean isInRange(String[] arrays, MsgParser msgParser, AlarmInfo info, MatchInfo matchInfo,
        ...
        if (ImcaptureUtil.isEmpty(info.getServiceCode())) {
            return false;
        }
    ...
    private boolean isServiceTypeInRange(AlarmInfo info, Map<String, ServiceInfo> map, MatchInfo matchInfo) {
        ServiceInfo serviceInfo = map.get(info.getServiceCode());
        if (serviceInfo == null) {
            return false;
        }
        ...
        return matchInfo.getServiceType().equals(serviceInfo.getServiceType());
    }
