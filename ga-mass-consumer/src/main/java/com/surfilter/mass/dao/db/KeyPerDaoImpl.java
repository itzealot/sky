...
public class KeyPerDaoImpl implements KeyPerDao {
	...
	private static final String I_ZD_PERSON_ALARM_INFO = "insert into zd_person_alarm_info(zd_person_id,zd_person_name,zd_person_mobile,zd_type,certificate_type,certificate_code,mac_id,match_type,match_child_type,match_value,service_code,xpoint,ypoint,start_time,end_time,alarm_time,data_type) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	private static final String I_CLUSTER_ALARM_RESULT = "insert into cluster_alarm_result(service_code,province_code,city_code,area_code,police_code,gang_list,first_alarm_time,last_alarm_time,create_time,zd_type) values(?,?,?,?,?,?,?,?,?,?)";
	...
	private static KeyPerDaoImpl instance = null;
	private int focusCounts;

	public static KeyPerDaoImpl getInstance(JdbcConfig jdbcConfig) {
		if (instance == null) {
			synchronized (KeyPerDaoImpl.class) {
				if (instance == null) {
					instance = new KeyPerDaoImpl(jdbcConfig);
				}
	...
	private KeyPerDaoImpl(JdbcConfig jdbcConfig) {
		this.pool = JdbcPool.getInstance(jdbcConfig);
		this.focusCounts = jdbcConfig.getFocusCounts();
	}
				...
				if (pstm != null) {
					...
					if (rs != null) {
						...
						while (rs.next()) {
							...
							if (isBlank(serviceType) && serviceType != null
							...
							MatchInfo matchInfo = new MatchInfo(storeId, macId, matchType, matchValue, matchChildType,
									...
									"1".equals(isMobileAlarm), "1".equals(isMailAlarm));

							Integer matchTypeValue = Integer.valueOf(matchType);

							if (MatchType.MAC.getCode() == matchTypeValue) {
								acinfo.add((matchValue + MatchType.MAC.getSimCode()).getBytes(), matchInfo);
							} else if (MatchType.PROTOCOL.getCode() == matchTypeValue) {
								acinfo.add((matchValue + MatchType.PROTOCOL.getSimCode()).getBytes(), matchInfo);
							} else if (MatchType.PHONE.getCode() == matchTypeValue) {
								acinfo.add((matchValue + MatchType.PHONE.getSimCode()).getBytes(), matchInfo);
							} else if (MatchType.IMEI.getCode() == matchTypeValue) {
								acinfo.add((matchValue + MatchType.IMEI.getSimCode()).getBytes(), matchInfo);
							} else if (MatchType.IMSI.getCode() == matchTypeValue) {
								acinfo.add((matchValue + MatchType.IMSI.getSimCode()).getBytes(), matchInfo);
							} else if (MatchType.CERT.getCode() == matchTypeValue) {
								acinfo.add((matchValue + matchChildType + MatchType.CERT.getSimCode()).getBytes(),
										matchInfo);
							} else if (MatchType.ACCOUNT.getCode() == matchTypeValue) {
								// match_type:match_value:match_child_type=>账号类数据解决后缀匹配的BUG
								acinfo.add((MatchType.ACCOUNT.getSimCode() + matchValue + matchChildType).getBytes(),
										matchInfo);
							}
						}
					}
	...
	private List<String> getAllKeyPer(String sql) {
		...
		if (conn != null) {
			ResultSet rs = null;
			PreparedStatement pstm = null;
			List<String> resultList = new ArrayList<String>(focusCounts);
				...
				if (rs != null) {
					while (rs.next()) {
						...
						AlarmInfo a = new AlarmInfo(matchValue, matchType, matchChildValue, null, null, startTime,
								...
								zdPersonId, zdPersonMobile, zd_person_name, zd_type, false, false);
	...
	@Override
	public void saveClusterAlarmResults(Map<String, Set<ClusterAlarmResult>> map) {
		Connection conn = pool.getConnection();
		...
		if (conn != null) {
			...
			try {
				...
				LOG.error("insert aluster alarm results, table=cluster_alarm_results fail.", e);
			} finally {
				JdbcUtils.close(pstm);
				pool.returnConnection(conn);
			}
