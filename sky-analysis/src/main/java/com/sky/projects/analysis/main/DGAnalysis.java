...
import com.sky.projects.analysis.service.DataAnalysis;
...
public class DGAnalysis implements IMain, Config, DBConfig, Serializable {
	...
	public void execute(String[] args) {
		...
		m_conf.set("spark.streaming.backpressure.enabled", "true");
		m_conf.set("spark.streaming.concurrentJobs", "5");
		...
		m_kafkaparam.put("bootstrap.servers", KAFKA_BROKERS);
		m_kafkaparam.put("broker.list", KAFKA_BROKERS);
		m_kafkaparam.put("auto.offset.reset", "smallest");
		m_kafkaparam.put("auto.commit.interval.ms", "5000");
		m_kafkaparam.put("fetch.message.max.bytes", "41943040");
		m_kafkaparam.put("zookeeper.connection.timeout.ms", "30000");
		m_kafkaparam.put("zookeeper.session.timeout.ms", "10000");
		m_kafkaparam.put("zookeeper.sync.time.ms", "10000");
		for (String topic : TOPICS.split("[,]")) {
			m_topics.put(topic, Integer.valueOf(KAFKA_MAX_PARTITIONS));
		}
		...
		DataAnalysis.execute(pairsDStream);
		m_streamContext.start();
		LOG.info("streaming context has started,waiting for termination....");
	...
	public static void updateOffsetToZk(JavaDStream<String> sourceStream, final String streamType, String zkConn,
			final String groupId) {
		sourceStream.foreachRDD(new Function<JavaRDD<String>, Void>() {
			@Override
			public Void call(JavaRDD<String> rdd) throws Exception {
				OffsetRange[] offsets = ((HasOffsetRanges) rdd.rdd()).offsetRanges();
				ZkClient zkClient = null;
				try {
					zkClient = new ZkClient(zkConn, 30000, 30000, new ZkUtils.StringSerializer());
					...
					DGAnalysis.LOG.error(streamType + " update offset to zk error!", e);
				} finally {
					try {
						ZkUtils.close(zkClient);
					} catch (IOException e) {
						DGAnalysis.LOG.error(streamType + " close zk error!", e);
					}
				}
				return null;
			}
